...
class LagrangeCoefficients():
    ...
    # ! ALGORITHM 2.3
    @classmethod
    def calculatePositionVelocityByAngle(cls, r_0 : np.ndarray, v_0 : np.ndarray, dtheta : float) -> list:
        ...
        r_0_m = linalg.norm(r_0)
        v_0_m = linalg.norm(v_0)
    ...
    # ! ALGORITHM 3.3
    @classmethod
    def calculateUniversalVariable(cls, r_0 : float, v_r0 : float, alpha : float, dt : float) -> float:
    ...
    @classmethod
    def calculateLagrangeCoefficients(cls, r_0 : float, alpha : float, dt : float, chi : float) -> list:
    ...
    # ! ALGORITHM 3.4
    @classmethod
    def calculatePositionVelocityByTime(cls, r_0 : np.ndarray, v_0 : np.ndarray, dt : float) -> list:
        ...
        chi = cls.calculateUniversalVariable(r_0_m, v_r0, alpha, dt)
        ...
        df_dt = np.sqrt(cls.mu) / (linalg.norm(r) * r_0_m) * (alpha * chi**3 * cls.S(z) - chi)
        
        dg_dt = 1 - chi**2 / linalg.norm(r) * cls.C(z)
...
if __name__  == '__main__':
    ...
    print('EXAMPLE 2.13\n')
    print(LagrangeCoefficients.calculatePositionVelocityByAngle(np.array([8182.4, -6865.9, 0]), np.array([0.47572, 8.8116, 0]), np.deg2rad(120)))
    print('-' * 40, '\n')
    
    print('EXAMPLE 3.6\n')
    print(LagrangeCoefficients.calculateUniversalVariable(10000, 3.0752, -5.0878e-5, 3600))
    print('-' * 40, '\n')
    
    print('EXAMPLE 3.7\n')
    print(LagrangeCoefficients.calculatePositionVelocityByTime(np.array([7000.0, -12124, 0]), np.array([2.6679, 4.6210, 0]), 3600))
    print('-' * 40, '\n')
    
    print('EXAMPLE 4.2\n')
    print(LagrangeCoefficients.calculatePositionVelocityByTime(np.array([1600, 5310, 3800]), np.array([-7.350, 0.4600, 2.470]), 3200))
    print('-' * 40, '\n')